...
x = [0,0,0,1,1,1,2,2,2]
y = [0,1,2,0,1,2,0,1,2] #Indicies for the subplot placements on the subplot figures
axs[0][0].set_xlim([0,111])
axs[0][0].set_ylim([0,28]) #Setting the counds to match with all the data

while j < len(data)/14:
    for k in pulsesper:
        for l in range(0,k):
            erow = srow+14 #Each pulse is 14 lines
            pulse = data.iloc[srow:erow] #Narrows out the data by pulse
            C_t += Cs*(pulse.iloc[0]+pulse.iloc[1]+pulse.iloc[2]+pulse.iloc[3])

            c = max(pulse.iloc[0]+pulse.iloc[1]+pulse.iloc[2]+pulse.iloc[3])
            ind = np.argmax(pulse.iloc[0]+pulse.iloc[1]+pulse.iloc[2]+pulse.iloc[3])+delay #Finds the index of max current. Delay is in index units
            inds.append(ind) #Just here for reference
            ...
            axs[x[loc]][y[loc]].plot(ind,htemp[ind],'*r',alpha=0.2) #Plots the magnetic field at the max current index
            
            H_t += htemp
            max_t.append(htemp[ind])
            htemp = []
            srow +=14 #Moving onto the next pulse
